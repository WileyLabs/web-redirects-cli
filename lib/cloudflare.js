...
const cloudflareGet = async (path, singleResult, page, results) => {
  ...
  try {
    ...
    if (singleResult) {
      ...
      // getWorkerKVValuesByDomain needs to return `response.data`
      // so return this if `response.data.result` not present
...
const getWorkerKVValuesByDomain = async (accountId, workerKvNamespace, domain) => 
  cloudflareGet(`/accounts/${accountId}/storage/kv/namespaces/${workerKvNamespace}/values/${domain}`, true);
...
const deleteWorkerDomain = async (accountId, domainId) => cloudflareUpdate(
  'delete',
