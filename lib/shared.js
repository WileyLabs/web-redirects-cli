...
// gather all zones from Cloudflare
exports.gatherZones = (accountId) => axios.get(`/zones?per_page=50&account.id=${accountId}`)
  .then((resp) => {
    const all_zones = [];
    ...
    if ('result_info' in resp.data) {
      ...
      if (possible_pages > 1) {
        ...
        const promises = [...Array(total_pages - 1).keys()]
          .map((i) => axios.get(`/zones?per_page=50&page=${i + 2}&account.id=${accountId}`));
